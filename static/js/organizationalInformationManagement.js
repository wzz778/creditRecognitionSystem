        ...
        .then((result) => {
            ...
            for (let i = 0; i < result.data.msg.data.length; i++) {
               ...
               <ul>
                    ...
                    <li>${i + 1}</li>
                    <li>
                        <div class="" style="display: none;">${result.data.msg.data[i].id}</div>
                        <button onclick="changeBoxShow(this)">修改</button>
                        <button onclick="deleteFn(this)">删除</button>
                        <div class="" style="display: none;">${result.data.msg.data[i].name}</div>
                    </li>
                </ul>
               `
            }
...
sureChange.onclick = function () {
    if (changeName.value == '' || changeName.value.replace(/(^\s*)|(\s*$)/g, "") == "") {
        swal('请输入组织名(不能为纯空格)')
        return
    }
    let obj = {
...
sureAdd.onclick = function () {
    if (addOrganizeName.value == '' || addOrganizeName.value.replace(/(^\s*)|(\s*$)/g, "") == "") {
        swal('请输入组织名(不能为纯空格)')
        return
    }
    let obj = {
